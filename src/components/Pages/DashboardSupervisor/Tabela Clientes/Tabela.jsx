    ...
    useEffect(() => {
      const fetchData = async () => {
        try {
          ...
          ordena.forEach(item => {
            const { codclient, comission, ...rest } = item;
    
            // Se o codclient já existe no objeto somaComissoes, soma a comissão
            if (soma[codclient]) {
              soma[codclient].comission += parseFloat(eval(comission).toFixed(2));
            } else {
              // Caso contrário, cria um novo registro com a comissão
              soma[codclient] = { ...rest, codclient, comission: parseFloat(eval(comission).toFixed(2)) };
            }
          });
          const dadosComComissoes = Object.values(soma);
          
          setDados(dadosComComissoes);
        } catch (error) {
